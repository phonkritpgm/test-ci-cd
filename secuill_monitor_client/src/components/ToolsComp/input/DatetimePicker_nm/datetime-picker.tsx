...
const DatetimePicker = (prop: any) => {
    
    const [ dateDefault, setDateDefault ] = useState(setDefaultValue(new Date().toString()));
    const [ dateValueOnChange, setValueOnChange ] = useState(new Date().toString());
    const toggleId = uuidv4();

    function setDefaultValue(inputDate: string): string {
        const dateObj = new Date(inputDate);
        const setDate: string = dateObj.getDate().toString().padStart(2,"0") + 
                                "/" + 
    ...
    const toggleDateInput = async (e: ChangeEvent) => {
        e.defaultPrevented;
        const dtp: any = e.target;
        setDateDefault(setDefaultValue(dtp.value));
        setValueOnChange(dtp.value);
    }

    const handleClickCalendar = (e: MouseEvent) => {
        e.preventDefault;
        const dtp: any = document.getElementById(toggleId);
        dtp.showPicker();
        dtp.showOpenFilePicker();
    }

    return(
        <React.Fragment>
            <div key={ uuidv4() } className={ styles.input_date_group }>
                <input className={ styles.rt_input }
                    key={ uuidv4() }
                    type="text"
                    id={ prop.id }
                    name={ prop.name }
                    ...
                    value={ prop.value == undefined ? dateDefault : prop.value }
                >     
                </input>          
                <div key={ uuidv4() } className={ styles.rt_dt_toggle } onClick={ (e: MouseEvent) => { handleClickCalendar(e) } }>
                    {/* (e: ChangeEvent) => { toggleDateInput(e); } */}
                    <input key={ uuidv4() } id={ toggleId } className={ styles.rt_input_toggle } type='date' value={ dateValueOnChange } onChange={ (e: ChangeEvent) => { toggleDateInput(e); prop.onChange(e); } }></input>
                    <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 29.237 29.237">
                    <g>
                        <g>
                            <path style={{fill: "#10002"}} d="M7.685,24.819H8.28v-2.131h3.688v2.131h0.596v-2.131h3.862v2.131h0.597v-2.131h4.109v2.131h0.595
