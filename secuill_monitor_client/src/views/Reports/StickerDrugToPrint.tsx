...
const DrugToPrintSticker = () => {
    ...
    const dataRefill = ():Array<objStickerType> => {
        const arr:Array<objStickerType> = [];
        let old_drug:string = "";
        dataDrugRefill.map((d)=>{
            if (old_drug != d.drugCode){
                arr.push(
                    {
                        drugName:d.drugName,
                        diff:d.diff,
    ...
    const handleCheckAllCheckboxHeader = (e: boolean) => {
        ...
        for (let index = 0; index < tr.length; index++) {
            ...
            const resultData: any = dataRefill().filter( (d: any) =>  d.no == dataNumber );
    ...
    return(
        <DrugListContainer>
            ...
            <PrintContentRef ref={componentRef} className="ref_to_print">
                <table>
                    <thead>
                        <tr><th></th></tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td>
                                ...
                                {
                                    objDrugSelected && 
                                    objDrugSelected.map( (data: objStickerType) => (
                                        <div className="page" style={{pageBreakAfter: "always"}}>
                                            <StickerFormat
                                                title="เติมยา "
                                                drugName={ data.drugName }
                                                description={ `จำนวนที่ต้องนำไปเติม : ${ data.diff }` }
            ...
            <TablePatientLists>
                ...
                <TableTBody>
                    {
                        dataRefill().map((data:objStickerType, i: number) => (
